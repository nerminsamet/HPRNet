...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    if dataset == 'coco_hp':
      ...
      self.W = 1242
      self.H = 375
    elif num_classes == 1 or dataset == 'coco_body':
      self.names = coco_body_class_name
      self.num_joints = 1
      self.num_face_landmarks = 68
      self.colors_hp = [(255, 0, 255)]
      self.colors_box = [(0, 255, 255)]
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default', text=''):
    ...
    txt = '{}{:.1f} id:{}'.format(self.names[cat], conf, text)
    # txt = 'id:{}'.format(text)
  ...
  def add_coco_face_lm(self, points, img_id='default', text=''):

    points = np.array(points, dtype=np.int32).reshape(self.num_face_landmarks, 2)

    for j in range(self.num_face_landmarks):
      cv2.circle(self.imgs[img_id],
                 (points[j, 0], points[j, 1]), 3, self.colors_hp[0], -1)

  def add_coco_face_hp(self, points, img_id='default', text=''):
    score = points[-1]
    points = np.array(points, dtype=np.int32)#.reshape(self.num_joints, 2)

    for j in range(self.num_joints):
      cv2.circle(self.imgs[img_id],
                 (points[0], points[1]), 3, self.colors_hp[j], -1)
    ...
    # face box
    cv2.rectangle(self.imgs[img_id], (points[2], points[3]), (points[4], points[5]), self.colors_box[0], 2)
    cv2.putText(self.imgs[img_id], txt, (points[2], points[3] - 2),
                font, 0.5, self.colors_box[0], thickness=1, lineType=cv2.LINE_AA)
     # for j, e in enumerate(self.edges):
    #   if points[e].min() > 0:
    #     cv2.line(self.imgs[img_id], (points[e[0], 0], points[e[0], 1]),
    #                   (points[e[1], 0], points[e[1], 1]), self.ec[j], 2,
