...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--arch', default='dla_34',
                             help='model architecture. dla_34 | hourglass')
  ...
  def parse(self, args=''):
    ...
    if opt.head_conv == -1: # init default head_conv
      opt.head_conv = 256 if ('dla' in opt.arch) or ('dlahough' in opt.arch) else 64
    opt.pad = 127 if ('hourglass' in opt.arch) or ('hourglasscontext' in opt.arch) else 31
    opt.num_stacks = 2 if opt.arch == 'hourglass' or opt.arch == 'hourglasscontext' else 1
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'landmark':
      opt.flip_idx = dataset.flip_idx
      opt.face_flip_idx = dataset.face_flip_idx
      opt.hand_flip_idx = dataset.hand_flip_idx
      opt.foot_flip_idx = dataset.foot_flip_idx
      opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 26*2, 'wh_face':2}
      opt.heads.update({'face_lms': 68*2 , 'hand_lms': 42*2})
      if opt.reg_offset:
          opt.heads.update({'reg': 2})
      if opt.hm_hp:
          opt.heads.update({'hm_hp': 26})
      if opt.reg_hp_offset:
          opt.heads.update({'hp_offset': 2})

    elif opt.task == 'multi_pose':
      opt.flip_idx = dataset.flip_idx
      opt.heads = {'hm': opt.num_classes, 'wh': 2, 'hps': 266}
      if opt.reg_offset:
        opt.heads.update({'reg': 2})
      if opt.hm_hp:
        opt.heads.update({'hm_hp': 133})
      if opt.reg_hp_offset:
        opt.heads.update({'hp_offset': 2})
    else:
  ...
  def init(self, args=''):
    default_dataset_info = {
      ...
      'landmark': {'default_resolution': [512, 512], 'num_classes': 1, 'num_landmarks': 5,
              'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
              'dataset': 'coco_body', 'flip_idx': [[0, 0]], 'num_face_landmarks': 68},
      'multi_pose': {
